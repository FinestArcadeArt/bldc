...
static float scaling_factor = 1.0; // Initialize scaling factor to 1.0
static float current_speed;
static float threshold_speed;
static float current_rpm;
static float current_max_rpm = 150000;
static float current_speed_last;
// static volatile float mixed_output_last = 0;
...
static THD_FUNCTION(pas_thread, arg)
{
	...
	for (;;)
	{
		...
		case PAS_CTRL_TYPE_BASIC:
			...
			// Calculate the threshold speed for reducing the output to 0
			threshold_speed = max_speed * 0.9; // Last 5% of the maximum speed

			if (pedal_rpm > (config.pedal_rpm_start + 1.0) && current_rpm < current_max_rpm)
			{
				if (current_speed != current_speed_last && output > 0.1 && current_speed > threshold_speed)
				{
					current_max_rpm = (max_speed * current_rpm) / current_speed;
					current_speed_last = current_speed;
				}
				// Mapping max rpm to last speed reading value only when current speed is over the threshold speed and output is significant enough
				if (current_speed > threshold_speed)
				{
					// calculating the scaling factor directly proportional to th remaining to max
					scaling_factor = (current_max_rpm - current_rpm) / (current_max_rpm);
				}
				else if (current_speed < threshold_speed)
				{
					scaling_factor = 1.0;
					current_max_rpm = 150000;
				}

				// Apply the scaling factor to the output
				output = config.current_scaling * sub_scaling * scaling_factor;
			}
			else
			{
				output = 0.0;
			}
			// add security for legal isse
...
// this had to be modified (CAN_TORQUE_SENSOR) because it's an Bafang M600 specific hardware definition (CANbus torque sensor) it may be included but in a other way.
		...
		// try to slow things when speed limiting and pedaling
		if (pedal_rpm > (config.pedal_rpm_start + 1.0) && current_speed > threshold_speed && fabsf(output) < fabsf(output_ramp))
			ramp_time = config.ramp_time_pos;
